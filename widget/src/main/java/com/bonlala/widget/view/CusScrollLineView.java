...
public class CusScrollLineView extends View {
    ...
    private float intervalX = 80; // 坐标刻度的间隔
    ...
    private final int backGroundColor = Color.WHITE; // view的背景颜色
...
//            Log.e(TAG, "--11-11---firstPointX=" + firstPointX + "---firstMinX=" + firstMinX + "---firstMaxX=" + firstMaxX);
    ...
    public CusScrollLineView(Context context) {
        super(context);
        initPaint();
        gestureDetector = new GestureDetector(context, new MyOnGestureListener());
    }

    public CusScrollLineView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initPaint();
        gestureDetector = new GestureDetector(context, new MyOnGestureListener());
    }

    public CusScrollLineView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawY(Canvas canvas) {
        ...
        for (int i = 0; i < lableCountY; i++) {
            ...
            String[] spArray = titleStr.split("\\.");
            String resultYStr = spArray[0];
